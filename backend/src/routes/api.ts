...
export const apiRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.get('/compare', async (request, reply) => {
    try {
      const { normalizedLevelId, roleFamilyId, locationId, adjustForCol } = request.query as {
  ...
  fastify.get('/level-mappings', async (request, reply) => {
    try {
      const { companyAId, companyBId } = request.query as {
        companyAId?: string;
        companyBId?: string;
      };
  ...
  fastify.get('/benchmark', async (request, reply) => {
    try {
      const { normalizedLevelId, roleFamilyId, locationId, totalComp, currency } = request.query as {
        normalizedLevelId?: string;
        roleFamilyId?: string;
        locationId?: string;
        totalComp?: string;
  ...
  fastify.post('/datapoints', async (request, reply) => {
    try {
      const body = request.body as {
        ...
        companyLevelId?: string; // fallback mapping ID
        sourceDesignation?: string;
        sourceYearsOfExperience?: number | string;
        roleFamilyId?: string;
        locationId?: string;
        base?: number;
        bonus?: number;
  ...
  fastify.get('/datapoints', async (request, reply) => {
    try {
      const { includeUnverified } = request.query as { includeUnverified?: string };
  ...
  fastify.patch('/datapoints/:id/verify', async (request, reply) => {
    try {
      ...
      const updatedDp = await dataPointRepo.updateVerification(id, status as VerificationStatus);
      return updatedDp;
    } catch (error: any) {
      reply.status(500).send({ error: error.message });
    }
